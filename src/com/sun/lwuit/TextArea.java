 ...
 *
 * You should have received a coy of the GNU General Public License version
...
public class TextArea extends Component {
    ...
    private int absAlign = isRTL() ? RIGHT : LEFT;
    ...
    private TextArea(String text, int maxSize, int rows, int columns, int constraint){
        ...
        if(columns <= 1 && rows != 1){
            throw new IllegalArgumentException("columns must be larger then 1");
        }
        this.rows = rows;
    ...
    private Vector getRowStrings() {
        if(rowStrings == null || widthForRowCalculations != getWidth() - getStyle().getPadding(false, RIGHT) - getStyle().getPadding(false, LEFT)){
            initRowString();
            setShouldCalcPreferredSize(true);
        }
        return rowStrings;
